#!/usr/bin/env python3
"""
generate_shorts_oxygen.py
Shorts #3 Target High RPM (US Tier-1):
Topik: What If Earth Lost Oxygen For Just 5 Seconds?
Voiceover: US English (Christopher Neural)
Subtitle: VTT langsung dari edge-tts (100% Sinkron)
Footage: 1080x1920 60fps Planet Earth from Space Pexels API
Output: /root/assets/shorts_earth_no_oxygen.mp4
"""
...
WORKDIR = "/root/projects/shorts_oxygen"
PEXELS_KEY = "YOUR_PEXELS_API_KEY"
EDGE_TTS = "/root/telegram_remote_bot/venv/bin/edge-tts"
TELEGRAM_SCRIPT = "/root/telegram_remote_bot/send_telegram.py"
OUT_MP4 = "/root/assets/shorts_earth_no_oxygen.mp4"

SCRIPT_LINES = [
    "What if Earth lost oxygen,",
    "for just five seconds?",
    "You might think holding your breath is easy,",
    "but the world would experience instant annihilation.",
    "Every concrete building on Earth,",
    "would immediately collapse into clouds of dust,",
    "because oxygen is the crucial chemical binder.",
    "The daytime sky would turn pitch black,",
    "and Earth's crust beneath your feet would crumble,",
    "since forty-five percent of our crust is oxygen.",
    "Could humanity survive five seconds of pure chaos?"
]
...
Title: Earth Without Oxygen High RPM Shorts
ScriptType: v4.00+
PlayResX: 1080
PlayResY: 1920
...
"""
    ...
    if not os.path.exists(footage_path):
        query = "planet earth space dark atmosphere"
    ...
    caption = (
        "🌍 *SHORTS #3 TIER-1 HIGH RPM (SIAP PUBLISH)*\n\n"
        "📌 *Title*: What If Earth Lost Oxygen For Just 5 Seconds? 🌍\n"
        "🌐 *Target Audience*: United States, UK, Canada, Australia\n"
        "💰 *Est. RPM*: $4.50 - $10.00 per 1,000 views\n"
        f"⏱ *Duration*: {duration:.1f}s (9:16 Full HD 1080p)\n"
        "🎙 *Voiceover*: US English (Christopher Neural)\n"
        "✨ *Subtitle*: 100% sinkron edge-tts VTT, dynamic gold/white styling\n\n"
        "Video Shorts #3 siap diupload ke YouTube Akun 2 (ZYLVEmedia02)."
    )

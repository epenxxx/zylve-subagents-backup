#!/usr/bin/env python3
"""
generate_high_rpm_shorts.py
Generator Otomatis YouTube Shorts Ber-RPM Tinggi (Bahasa Inggris US):
- Topik Sains/Misteri/Fakta Dunia
- Voiceover US English sinematik via edge-tts
- Subtitle VTT diekstrak langsung dari edge-tts (100% SINKRON DENGAN SUARA)
- Subtitle ASS otomatis eye-tracking (kuning/putih bergantian, border tebal)
- Footage Vertikal 9:16 Full HD 1080x1920 resmi Pexels API
"""
...
OUT_MP4 = "/root/assets/shorts_mariana_trench.mp4"

SCRIPT_LINES = [
    "At the bottom of the Mariana Trench,",
    "the pressure is over one thousand times greater than at the surface.",
    "It is enough to crush a heavy submarine,",
    "like an empty soda can.",
    "It is pitch black and freezing cold,",
    "yet alien-like creatures survive here in absolute silence.",
    "More people have walked on the moon,",
    "than visited the deepest point of our ocean.",
    "Would you dare to go down there?"
]
...
def run():
    ...
    # 1. Tulis naskah baris per baris
...
Title: Mariana Trench High RPM Shorts Synchronized
ScriptType: v4.00+
PlayResX: 1080
PlayResY: 1920
...
"""
    ...
    # 4. Unduh Footage Vertikal 1080x1920 Pexels API jika belum ada
    print("[3/5] Memeriksa footage vertikal 1080p resmi via Pexels API...")
    footage_path = os.path.join(WORKDIR, "raw_footage.mp4")
    if not os.path.exists(footage_path):
        query = "deep ocean dark underwater"
    ...
    caption = (
        "🌊 *SHORTS TIER-1 HIGH RPM (REVISI SUBTITLE 100% SINKRON)*\n\n"
        "📌 *Title*: The Terrifying Truth About the Mariana Trench 🌊\n"
        "🌐 *Target Audience*: United States, UK, Canada, Australia\n"
        "💰 *Est. RPM*: $3.00 - $8.00 per 1,000 views\n"
        ...
        "Video hasil render siap diupload ulang ke YouTube Akun 2."
    )
